...
@section('content')
<form method="POST" action="{{ route('productimages.update', $productimage) }}" accept-charset="UTF-8" id="form" autocomplete="off" class="needs-validation" enctype="multipart/form-data">
	@csrf
	@method('PATCH')
	<div class="card">
		<div class="card-header">
			Edit Image for <strong>{!! $productimage->product->product !!}</strong>
		</div>
		<div class="card-body">
		...
		<div class="card-footer d-flex justify-content-end">
			...
			<a href="{{ route('products.edit', $productimage->product->slug) }}" class="btn btn-sm btn-outline-secondary me-1">Cancel</a>
		</div>
	</div>
</form>
@endsection
